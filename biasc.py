...
def write_grib_message(fp, args, analysistime, forecasttime, data):
    ...
    for j in range(0, len(data)):
        ...
        if tosp is not None:
            ...
            ecc.codes_set(h, "yearOfEndOfOverallTimeInterval", int(forecasttime[i].strftime('%Y')))
            ecc.codes_set(h, "monthOfEndOfOverallTimeInterval", int(forecasttime[i].strftime('%m')))
            ecc.codes_set(h, "dayOfEndOfOverallTimeInterval", int(forecasttime[i].strftime('%d')))
            ecc.codes_set(h, "hourOfEndOfOverallTimeInterval", int(forecasttime[i].strftime('%H')))
